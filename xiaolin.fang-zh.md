...
Agentic Job Scraper 是一个自动化系统，从 Telegram 频道、RSS 订阅源（如 V2EX、电鸭社区）和招聘网站（Bossjob.com、LinkedIn）获取软件开发职位信息，使用本地 AI (Ollama) 进行分析，并在现代化的 Web 界面中展示。非常适合寻找远程机会的开发者或监控市场趋势的招聘人员。
...
- **🔗 LinkedIn 集成** — 基于 Playwright 的抓取，支持公开职位搜索和认证（注入 Cookie）两种模式，结构化提取并自动过滤软件工程岗位
...
- **🍪 Cookie 扩展** — Chrome 扩展，一键导出 Bossjob.com / LinkedIn Cookie
...
适用于现有来源：Telegram 频道、Bossjob.com、LinkedIn、电鸭社区、V2EX。

自动投递职位功能**未实现**；自主性仅限于优化现有来源。

## 🚀 计划功能

- [ ] 扩展职位板支持（Bossjob.com 和 LinkedIn 之外的更多招聘网站）
- [ ] LinkedIn Cookie Chrome 扩展（类似 Bossjob 导出工具）
- [ ] 更多 i18n 语言
- [ ] Docker 容器化
- [ ] CI/CD 管道
