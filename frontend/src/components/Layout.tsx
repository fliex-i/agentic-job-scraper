...
interface WebSocketProgressContextType {
  ...
  currentAnalyzingMessage: Record<
    string,
    { message_id?: number; message_text: string; message_preview: string }
  >;
  statsUpdate: {
...
const WebSocketProgressContext =
  createContext<WebSocketProgressContextType | null>(null);

export const useWebSocketProgress = () => {
  const context = useContext(WebSocketProgressContext);
  if (!context)
    throw new Error(
      "useWebSocketProgress must be used within WebSocketProgressProvider",
    );
  return context;
};
...
const WebSocketProgressProvider = ({
  children,
}: {
  ...
  const [channelProgress, setChannelProgress] = useState<
    Record<string, { analyzed: number; total: number }>
  >({});
  const [operations, setOperations] = useState<
    Record<string, { type: string; status: string }>
  >({});
  ...
  >([]);
  const [stoppingChannels, setStoppingChannels] = useState<
    Record<string, boolean>
  >({});
  const [tokenUsage, setTokenUsage] = useState<
    Record<string, { input: number; output: number; total: number }>
  >({});
  ...
  const [cronStatus, setCronStatus] = useState<{ running: boolean } | null>(
    null,
  );
  ...
  const [channelUpdates, setChannelUpdates] = useState<Array<{
    id: number;
    username: string;
    is_listened: number;
    telegram_account_id: number | null;
  ...
  const showNotification = (title: string, body: string) => {
    if ("Notification" in window && Notification.permission === "granted") {
      ...
      if (
        ...
        now - lastNotificationRef.current[key] < 5000
      ) {
        return;
      }
        ...
        if (data.operations && data.operations.length > 0) {
          ...
          data.operations.forEach((op: any) => {
            if (
              ...
              const opType =
                op.operation_type === "analyze" ? "analyze" : "fetch";
  ...
  useEffect(() => {
    localStorage.setItem(
      "ws_channel_progress",
      JSON.stringify(channelProgress),
    );
  }, [channelProgress]);
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (
              channel &&
              (data.type === "analyze_start" || data.type === "fetch_start")
            ) {
              // Start operation
              const opType =
                data.type === "analyze_start" ? "analyze" : "fetch";
              setOperations((prev) => ({
                ...prev,
              ...
              channel &&
              (data.type === "analyze_complete" ||
                data.type === "fetch_complete" ||
                data.type === "error")
            ) {
              ...
              data.type === "bulk_fetch_complete" ||
              data.type === "bulk_fetch_stopped"
            ) {
              // Handle bulk fetch completion or stop
              setBulkOperations((prev) =>
                prev.filter((op) => op.id !== String(data.operation_id)),
              );
              if (data.type === "bulk_fetch_complete") {
                showNotification(
                  "Bulk Fetch Complete",
                  `Fetched ${data.total_new_messages} new messages`,
                );
              }
            }
          } catch (e) {
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const navLinks = [
    ...
    { path: "/channels", label: t("nav.channels"), icon: Radio },
    {
      ...
      label: t("nav.telegramAccounts"),
      icon: Radio,
    },
    { path: "/autonomous", label: "Autonomous", icon: Bot },
    {
      path: "/apply-records",
      label: t("nav.applyRecords"),
      icon: ClipboardList,
    },
  ];
