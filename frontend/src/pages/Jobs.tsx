...
const getJobPublishedDisplay = (job: Job) => {
  if (job.source_published_at)
    return new Date(job.source_published_at).toLocaleString();
  if (job.source_published_text) return job.source_published_text;
  if (job.message?.date) return new Date(job.message.date).toLocaleString();
  return null;
};

const getJobUpdatedDisplay = (job: Job) => {
  if (job.source_updated_at)
    return new Date(job.source_updated_at).toLocaleString();
  if (job.source_updated_text) return job.source_updated_text;
  return null;
};
...
const Jobs = () => {
  ...
  const toggleApplied = async (id: number) => {
    ...
    // Optimistic update
    setJobs((prevJobs) =>
      prevJobs.map((j) => (j.id === id ? { ...j, is_applied: true } : j)),
    );
    if (selectedJob?.id === id) {
      setSelectedJob((prev) => (prev ? { ...prev, is_applied: true } : null));
    }
    try {
      await withLoading(`toggle-${id}`, () =>
        api.toggleJobApplied(id, jobNotes),
      );
      loadJobs();
      setJobNotes("");
  ...
  const exportJobs = () => {
    ...
    const rows = jobs.map((job) => {
      ...
      return [
        ...
        getJobPublishedDisplay(job) || "",
        getJobUpdatedDisplay(job) || "",
      ];
    });
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <span
                              className={`font-medium text-base truncate ${isSelected ? "text-primary" : ""}`}
                            >
                              {job.title ||
                                job.message?.text?.substring(0, 60) +
                                  (job.message?.text &&
                                  job.message.text.length > 60
                                    ? "..."
                                    : "") ||
                                t("jobs.untitledJob")}
                            </span>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center text-lg sm:text-xl font-bold text-primary-foreground shrink-0">
                      {getInitials(
                        selectedJob.company || selectedJob.title || "Job",
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap gap-2 sm:gap-2.5 mb-2 max-w-full">
                        <Badge
                          variant={
                            selectedJob.is_applied ? "default" : "secondary"
                          }
                        ...
                        {selectedJob.source_type && (
                          <Badge
                            variant="outline"
                            className="text-xs sm:text-sm px-2.5 py-0.5 sm:px-3 sm:py-1 shrink-0"
                          >
                            {selectedJob.source_type === "telegram"
                              ? t("common.telegram")
                              : t("common.website")}
                          </Badge>
                        )}
                        {selectedJob.is_remote && (
                          <Badge className="bg-teal-100 text-teal-700 hover:bg-teal-100 text-sm px-3 py-1 shrink-0">
                            {t("jobs.remote")}
                          </Badge>
                        )}
                      ...
                      <h2 className="text-lg sm:text-xl font-bold truncate">
                        {selectedJob.title ||
                          selectedJob.message?.text?.substring(0, 80) +
                            (selectedJob.message?.text &&
                            selectedJob.message.text.length > 80
                              ? "..."
                              : "") ||
                          t("jobs.untitledJob")}
                      </h2>
                      <p className="text-xs sm:text-sm text-muted-foreground flex items-center gap-1 mt-0.5 flex-wrap">
                        ...
                        <span className="truncate">
                          {selectedJob.company || t("jobs.unknownCompany")}
                        </span>
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] sm:text-xs text-gray-500 font-medium">
                          {t("jobs.companyWebsite")}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-900 truncate group-hover:text-primary transition-colors">
                          {selectedJob.company_link}
                        </p>
                      </div>
                    </a>
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        {getSkills(selectedJob).map((skill, idx) => (
                          <Badge
                            key={idx}
                            variant="secondary"
                            className="px-4 py-1.5 text-sm font-medium bg-primary/10 text-primary hover:bg-primary/20"
                          >
                            {skill}
                          </Badge>
                        ))}
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-2 sm:mb-3">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const text =
                            selectedJob?.message?.text?.replace(
                              /<[^>]*>/g,
                              "",
                            ) || "";
                    ...
                    <div
                      className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap break-words prose prose-sm max-w-none"
                      dangerouslySetInnerHTML={{
                        __html:
                          selectedJob.message?.text || t("jobs.noTextContent"),
                      }}
                    />
                    <div className="mt-3 pt-3 border-t border-gray-200 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                      <span className="flex items-center gap-1">
                        <MessageSquare className="w-3 h-3" />
                        {selectedJob.channel_name ||
                          selectedJob.channel?.username ||
                          t("common.unknown")}
                      </span>
                      <span className="flex items-center gap-1">
                        <Calendar className="w-3 h-3" />
                        {getJobPublishedDisplay(selectedJob) ||
                          t("common.unknown")}
                      </span>
                      {getJobUpdatedDisplay(selectedJob) && (
                        <span className="flex items-center gap-1">
                          <Calendar className="w-3 h-3" />
                          Updated: {getJobUpdatedDisplay(selectedJob)}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedJob.notes !== undefined && (
                    <div className="bg-yellow-50/50 border border-yellow-100 rounded-xl p-4">
                      ...
                      <p className="text-sm text-gray-700 leading-relaxed break-words">
                        {selectedJob.notes || t("jobs.noNotes")}
                      </p>
                    </div>
                  )}
                </div>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t("jobs.deleteWarning")}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
